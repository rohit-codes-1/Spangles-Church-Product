const WomenFellowExpense = require("../Schema/WomenExpense");
...
exports.addExpense = async (req, res) => {
  try {
    const expense = new WomenFellowExpense(req.body);
    await expense.save();
...
exports.getExpenses = async (req, res) => {
  try {
    ...
    const expenses = await WomenFellowExpense.find(filter)
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limit);

    const total = await WomenFellowExpense.countDocuments(filter);
...
exports.getExpenseById = async (req, res) => {
  try {
    const expense = await WomenFellowExpense.findById(req.params.id);
...
exports.updateExpense = async (req, res) => {
  try {
    const expense = await WomenFellowExpense.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (!expense) return res.status(404).json({ message: "Expense not found" });
...
exports.deleteExpense = async (req, res) => {
  try {
    const expense = await WomenFellowExpense.findByIdAndDelete(req.params.id);
    if (!expense) return res.status(404).json({ message: "Expense not found" });
...
exports.closeBill = async (req, res) => {
  try {
    ...
    const expense = await WomenFellowExpense.findById(id);
...
// 📂 Multer setup for Women Fellowship bills
const uploadDir = "uploads/womenfellowbills";
