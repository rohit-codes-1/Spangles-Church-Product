const CoupleExpense = require("../Schema/CoupleExpense");
...
exports.addExpense = async (req, res) => {
  try {
    const expense = new CoupleExpense(req.body);
    await expense.save();
...
exports.getExpenses = async (req, res) => {
  try {
    ...
    if (req.query.approval_status && req.query.approval_status !== "All") filter.approval_status = req.query.approval_status;
    if (req.query.billStatus && req.query.billStatus !== "All") filter.billStatus = req.query.billStatus;
    if (req.query.search) {
      filter.$or = [
    ...
    const expenses = await CoupleExpense.find(filter)
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limit);

    const total = await CoupleExpense.countDocuments(filter);
...
exports.getExpenseById = async (req, res) => {
  try {
    const expense = await CoupleExpense.findById(req.params.id);
...
exports.updateExpense = async (req, res) => {
  try {
    const expense = await CoupleExpense.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (!expense) return res.status(404).json({ message: "Expense not found" });
...
exports.deleteExpense = async (req, res) => {
  try {
    const expense = await CoupleExpense.findByIdAndDelete(req.params.id);
    if (!expense) return res.status(404).json({ message: "Expense not found" });
...
exports.closeBill = async (req, res) => {
  try {
    ...
    const expense = await CoupleExpense.findById(id);
...
// 📂 Multer setup for Couple bills
const uploadDir = "uploads/couplebills";
