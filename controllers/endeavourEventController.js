...
exports.addEvent = async (req, res) => {
  try {
    ...
    // ✅ Expand grouped class names (Primary, Junior, etc.)
    let expandedClassEvents = [];

    for (const cls of classEvents) {
      // Find all sections in the DB that match this group
      const sections = await EndeavourClass.find({ class_name: cls.className });

      if (sections.length === 0) {
        // if no sections found, fallback to original group
        expandedClassEvents.push(cls);
      } else {
        // Duplicate competitions for each section
...
//     // Find event by ID
//     const event = await EndeavourEvent.findById(id);
//     if (!event) {
//       return res.status(404).json({ status: "Failed", message: "Event not found" });
//     }
...
exports.updateEvent = async (req, res) => {
  try {
    ...
    // Expand grouped class names to all sections
    let expandedClassEvents = [];
...
exports.addParticipants = async (req, res) => {
  try {
    ...
    // load event
    const event = await EndeavourEvent.findById(eventId).lean().exec();
    if (!event) {
      return res.status(404).json({ status: "Failed", message: "Event not found" });
    }
