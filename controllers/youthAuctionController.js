const YouthAuction = require("../Schema/YouthAuctionSchema");
const YouthAuctionPayment = require("../Schema/YouthAuctionPaymentSchema");

// ➤ Add Auction
exports.addAuction = async (req, res) => {
  try {
    const auction = new YouthAuction(req.body);
    await auction.save();
    res.status(201).json({ success: true, message: "Youth Auction created", auction });
  } catch (error) {
    res.status(400).json({ success: false, message: error.message });
  }
};

// ➤ Get All Auctions (with pagination & filters)
exports.getAuctions = async (req, res) => {
  try {
    ...
    const auctions = await YouthAuction.find(query)
      .sort({ createdAt: -1 })
      .skip((page - 1) * limit)
      .limit(parseInt(limit));

    const total = await YouthAuction.countDocuments(query);
...
exports.getAuctionById = async (req, res) => {
  try {
    const auction = await YouthAuction.findById(req.params.id);
...
exports.updateAuction = async (req, res) => {
  try {
    const auction = await YouthAuction.findByIdAndUpdate(req.params.id, req.body, {
      new: true,
      runValidators: true,
    });
    if (!auction) return res.status(404).json({ success: false, message: "Auction not found" });
...
exports.deleteAuction = async (req, res) => {
  try {
    const auction = await YouthAuction.findByIdAndDelete(req.params.id);
    if (!auction) return res.status(404).json({ success: false, message: "Auction not found" });
...
// ➤ Internal helper for report
async function buildBuyerReport({ buyerId, buyerPhone }) {
  const filter = buyerId ? { buyerId } : { buyerPhone };
  const auctions = await YouthAuction.find(filter).lean();
  const payments = await YouthAuctionPayment.find(filter).lean();
...
// ➤ Youth Auction Report
exports.getYouthAuctionReportByBuyer = async (req, res) => {
  try {
    const { search = "", page = 1, limit = 10 } = req.query;
    const auctions = await YouthAuction.find().lean();
    ...
    console.error("Youth Auction Report error:", err);
    res.status(500).json({ message: "Server error", error: err.message });
  }
};

// ➤ Add Payment
exports.addYouthAuctionPayment = async (req, res) => {
  try {
    const { auctionId, amountPaid } = req.body;
    const auction = await YouthAuction.findById(auctionId);
    ...
    const payment = new YouthAuctionPayment({
      youthAuctionId: auction._id,
    ...
    console.error("Youth Auction Payment error:", err);
    res.status(500).json({ message: "Server error", error: err.message });
  }
};
