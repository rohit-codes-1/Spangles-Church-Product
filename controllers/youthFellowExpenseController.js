const YouthExpense = require("../Schema/YouthExpense");
...
exports.addExpense = async (req, res) => {
  try {
    const expense = new YouthExpense(req.body);
    await expense.save();
...
exports.getExpenses = async (req, res) => {
  try {
    ...
    const expenses = await YouthExpense.find(filter)
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limit);

    const total = await YouthExpense.countDocuments(filter);
...
exports.getExpenseById = async (req, res) => {
  try {
    const expense = await YouthExpense.findById(req.params.id);
...
exports.updateExpense = async (req, res) => {
  try {
    const expense = await YouthExpense.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (!expense) return res.status(404).json({ message: "Expense not found" });
...
exports.deleteExpense = async (req, res) => {
  try {
    const expense = await YouthExpense.findByIdAndDelete(req.params.id);
    if (!expense) return res.status(404).json({ message: "Expense not found" });
...
exports.closeBill = async (req, res) => {
  try {
    ...
    const expense = await YouthExpense.findById(id);
...
// 📂 Multer setup for Youth bills
const uploadDir = "uploads/youthbills";
