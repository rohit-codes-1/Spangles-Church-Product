//sundaySchoolExpenseController.js

const SundaySchoolExpense = require("../Schema/SundaySchoolExpense");

// ➕ Add Expense
exports.addExpense = async (req, res) => {
  try {
    const expense = new SundaySchoolExpense(req.body);
    await expense.save();
...
exports.getExpenses = async (req, res) => {
  try {
    ...
    const expenses = await SundaySchoolExpense.find(filter)
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limit);

    const total = await SundaySchoolExpense.countDocuments(filter);
...
exports.getExpenseById = async (req, res) => {
  try {
    const expense = await SundaySchoolExpense.findById(req.params.id);
...
exports.updateExpense = async (req, res) => {
  try {
    const expense = await SundaySchoolExpense.findByIdAndUpdate(
      req.params.id,
      req.body,
      { new: true }
    );
...
exports.deleteExpense = async (req, res) => {
  try {
    const expense = await SundaySchoolExpense.findByIdAndDelete(req.params.id);
    if (!expense) return res.status(404).json({ message: "Expense not found" });
...
exports.closeBill = async (req, res) => {
  try {
    ...
    const expense = await SundaySchoolExpense.findById(id);
    if (!expense) {
...
const uploadDir = "uploads/sundaysclbills";
