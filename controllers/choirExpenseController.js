// choirExpenseController.js

const ChoirExpense = require("../Schema/ChoirExpense");

// ➕ Add Expense
exports.addExpense = async (req, res) => {
  try {
    const expense = new ChoirExpense(req.body);
    await expense.save();
...
exports.getExpenses = async (req, res) => {
  try {
    ...
    const expenses = await ChoirExpense.find(filter)
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limit);

    const total = await ChoirExpense.countDocuments(filter);
...
exports.getExpenseById = async (req, res) => {
  try {
    const expense = await ChoirExpense.findById(req.params.id);
...
exports.updateExpense = async (req, res) => {
  try {
    const expense = await ChoirExpense.findByIdAndUpdate(
      req.params.id,
      req.body,
      { new: true }
    );
...
exports.deleteExpense = async (req, res) => {
  try {
    const expense = await ChoirExpense.findByIdAndDelete(req.params.id);
    if (!expense) return res.status(404).json({ message: "Expense not found" });
...
exports.closeBill = async (req, res) => {
  try {
    ...
    const expense = await ChoirExpense.findById(id);
    if (!expense) {
...
const uploadDir = "uploads/choirbills";
