// endeavourExpenseController.js

const EndeavourExpense = require("../Schema/EndeavourExpense");

// ➕ Add Expense
exports.addExpense = async (req, res) => {
  try {
    const expense = new EndeavourExpense(req.body);
    await expense.save();
...
exports.getExpenses = async (req, res) => {
  try {
    ...
    if (req.query.status && req.query.status !== "All") {
      filter.status = req.query.status; // Paid / Unpaid
    }
    ...
    if (req.query.billStatus && req.query.billStatus !== "All") {
      filter.billStatus = req.query.billStatus; // Open / Closed
    }
    if (req.query.search) {
      filter.$or = [
    ...
    const expenses = await EndeavourExpense.find(filter)
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limit);

    const total = await EndeavourExpense.countDocuments(filter);
...
exports.getExpenseById = async (req, res) => {
  try {
    const expense = await EndeavourExpense.findById(req.params.id);
...
// ✏️ Update Expense (e.g., approve/reject)
exports.updateExpense = async (req, res) => {
  try {
    const expense = await EndeavourExpense.findByIdAndUpdate(
      req.params.id,
      req.body,
      { new: true }
    );
...
exports.deleteExpense = async (req, res) => {
  try {
    const expense = await EndeavourExpense.findByIdAndDelete(req.params.id);
    if (!expense) return res.status(404).json({ message: "Expense not found" });
...
exports.closeBill = async (req, res) => {
  try {
    ...
    if (!billName || !billNo || !billFile) {
      return res
        .status(400)
        .json({ status: "Failed", message: "Bill name, number, and file are required" });
    }

    const expense = await EndeavourExpense.findById(id);
    if (!expense) {
    ...
    res
      .status(200)
      .json({ status: "Success", message: "Bill closed successfully", data: expense });
  } catch (err) {
...
const uploadDir = "uploads/endeavourbills";
